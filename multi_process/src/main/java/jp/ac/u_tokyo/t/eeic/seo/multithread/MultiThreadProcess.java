package jp.ac.u_tokyo.t.eeic.seo.multithread;

import java.util.Collection;
import java.util.LinkedList;
import java.util.Queue;

public abstract class MultiThreadProcess<T,P>{
     ...
     */
    protected final void start(int threadNum){
    ...
    protected final void enqueue(Collection<T> tasks){
        for ( T task : tasks ){
            enqueue(task);
        }
    }
     ...
     */
    protected final synchronized void enqueue(T task){
        if ( mIsProcessing ){
            if ( mIsQueuing ) {
                if (task == null) {
     ...
     * 待ち行列に残っているタスクを処理し終えた段階で{@link #onComplete(int, long)}がコールバックされ、
     * このコールバック内での処理を終えたのちにスレッドは戻ります
     * <strong>NOTE </strong>{@link #start(int)}で処理開始を宣言したあとに一回のみ呼び出せます.
     * それ以外の不正なタイミングで呼ぶと例外を投げます.
     */
    protected final synchronized void stopEnqueue(){
     ...
     */
    protected abstract class ProcessThread extends Thread{
    ...
    private synchronized void onProcessFinish(){
        ...
        if ( mThreadCnt == 0 ){
            ...
            //let thread run that called onStopEnqueue
            notifyAll();
        }
    }
